                ...
                <Transition enter-active-class="transition-all duration-200 ease-out"
                    ...
                    <div v-if="modelValue"
                        ...
                        <div class="w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center"
                            ...
                            <AppIcon :name="icon" :size="26" :class="variantClasses.iconColor" />
                        </div>
...
const icon = computed(() => {
    const map = { danger: 'triangle-alert', warning: 'circle-alert', success: 'circle-check', info: 'info' }
    return map[props.variant ?? 'warning']
})

const variantClasses = computed(() => {
    const map = {
        danger: { iconBg: 'bg-red-50', iconColor: 'text-red-500', confirmBtn: 'bg-red-600 hover:bg-red-700' },
        warning: { iconBg: 'bg-amber-50', iconColor: 'text-amber-500', confirmBtn: 'bg-amber-500 hover:bg-amber-600' },
        success: { iconBg: 'bg-green-50', iconColor: 'text-green-500', confirmBtn: 'bg-green-600 hover:bg-green-700' },
        info: { iconBg: 'bg-blue-50', iconColor: 'text-blue-500', confirmBtn: 'bg-blue-600 hover:bg-blue-700' },
    }
    return map[props.variant ?? 'warning']
})
</script>