...
import { Calendar, Banknote, StickyNote, Heart } from 'lucide-vue-next'
...
import NuevoPedidoCatalogoCarrito from '@/components/admin/NuevoPedidoCatalogoCarrito.vue'
import {
...
const despachoModal = reactive({
  ...
  // Antes de mostrar "Solicitar", hay que confirmar que no exista ya un
  // despacho activo para este pedido — si no se checa, se puede pedir
  // un repartidor dos veces sin darse cuenta.
  checkingEstado: false,
  estadoActual: null as Record<string, any> | null,
})
...
// ── Despacho ──────────────────────────────────────────────
async function abrirDespachoModal(order: AdminOrder) {
  if (!can.value.writeOrders) return
  despachoModal.order = order; despachoModal.show = true
  despachoModal.loading = false; despachoModal.error = ''
  despachoModal.estadoActual = null

  // Antes de ofrecer "Solicitar", hay que ver si ya hay uno activo —
  // sin esto, se podía pedir un repartidor dos veces para el mismo
  // pedido sin ningún aviso.
  despachoModal.checkingEstado = true
  try {
    const { data } = await api.get(`/admin/despachos/${order.id}/estado`)
    despachoModal.estadoActual = data.data ?? null
  } catch {
    // 404 = no hay despacho activo todavía, es el caso normal — no es
    // un error que haya que mostrarle al usuario.
    despachoModal.estadoActual = null
  } finally {
    despachoModal.checkingEstado = false
  }
}
...
async function cancelarDespachoActivo() {
  if (!despachoModal.order) return
  despachoModal.loading = true; despachoModal.error = ''
  try {
    await api.post(`/admin/despachos/${despachoModal.order.id}/cancelar`)
    despachoModal.show = false
  } catch (e: any) {
    despachoModal.error = e.response?.data?.message ?? 'Error al cancelar el despacho'
  } finally {
    despachoModal.loading = false
  }
}
